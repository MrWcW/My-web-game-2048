document.addEventListener("DOMContentLoaded", function() {
  ...
  // Functions to start 2048 and Tetris games
  function start2048Game() {
    document.getElementById('mobilewrap').style.display = 'block'; // Show 2048 game
    document.getElementById('tetrisGame').style.display = 'none'; // Hide Tetris
    restoreField(); // Initialize 2048 game state
    init(); // Initialize 2048
  }

  function initializeTetrisGame() {
    document.getElementById('mobilewrap').style.display = 'none'; // Hide 2048
    document.getElementById('tetrisGame').style.display = 'block'; // Show Tetris
    // Add Tetris game initialization logic here if needed
  }

  // Menu options click handlers
  document.getElementById('tetrisOption').addEventListener('click', function() {
    initializeTetrisGame();
    dropdownMenu.classList.remove('show'); // Hide dropdown menu after selection
  });

  document.getElementById('gameOption').addEventListener('click', function() {
    start2048Game();
    dropdownMenu.classList.remove('show'); // Hide dropdown menu after selection
  });
  ...
  hammertime.on('swipeleft', function(ev) {
    moveDirection(37); // Left arrow key (move left)
  });

  hammertime.on('swiperight', function(ev) {
    moveDirection(39); // Right arrow key (move right)
  });

  hammertime.on('swipeup', function(ev) {
    moveDirection(38); // Up arrow key (move up)
  });

  hammertime.on('swipedown', function(ev) {
    moveDirection(40); // Down arrow key (move down)
  });
